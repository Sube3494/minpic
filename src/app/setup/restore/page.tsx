          ...
          >
            <div className="max-w-md w-full p-8 space-y-8 text-center">
              ...
              <div className="space-y-4">
                <h2 className="text-2xl font-bold tracking-tight">系统正在深度还原中</h2>
                <p className="text-sm text-muted-foreground leading-relaxed h-5">
                  {restoreStep}
                </p>
              </div>

              <div className="space-y-3">
                <div className="flex justify-between text-xs font-medium uppercase tracking-tighter opacity-70">
                  <span>Restore Progress</span>
                  <span>{Math.round(progress)}%</span>
                </div>
                <Progress value={progress} className="h-2 bg-zinc-200 dark:bg-zinc-800 overflow-hidden relative">
                  <div className="absolute inset-0 bg-linear-to-r from-transparent via-white/20 to-transparent animate-shimmer" />
                </Progress>
              </div>

              <div className="pt-4 flex items-center justify-center gap-3 text-xs text-muted-foreground italic">
                <Loader2 className="w-3 h-3 animate-spin" />
                此过程大约需要 10-60 秒，请勿关闭页面...
              </div>
            </div>
          </motion.div>
        )}
